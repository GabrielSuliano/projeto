...
function writeFinalSky() {
  closeGrandFinale();
  const finalCopy = $(".finale-copy");
  finalCopy.style.transition = "opacity 260ms ease, transform 260ms ease";
  finalCopy.style.opacity = "0";
  finalCopy.style.transform = "translateY(18px)";
  finalCopy.style.pointerEvents = "none";

  $("#final").scrollIntoView({ behavior: "smooth", block: "center" });

  window.setTimeout(() => {
    universe.formText("JULINHA", true, { yRatio: window.innerWidth < 720 ? 0.48 : 0.46 });
    universe.fireworks();
    playChime(true);
  }, 520);
  ...
  window.setTimeout(() => {
    ...
    finalCopy.style.transform = "translateY(0)";
    finalCopy.style.pointerEvents = "";
  }, 6200);
}
...
const universe = (() => {
  ...
  function resize() {
    ...
    if (targetPoints.length) {
      formText(targetText || "JULINHA", false, targetOptions);
    }
  }
  ...
  function formText(text, announce = true, options = {}) {
    targetText = text;
    ...
    const fontSize = options.fontSize || (width < 720 ? 62 : Math.min(148, Math.floor(width / 7.4)));
    const textY = height * (options.yRatio || (width < 720 ? 0.5 : 0.5));
  ...
  function launchFireworks() {
    const colors = ["#ffcf75", "#67e7d1", "#ff6f61", "#fff7e6", "#e94d68"];
    const bursts = width < 720 ? 6 : 9;

    for (let index = 0; index < bursts; index += 1) {
      window.setTimeout(() => {
        const x = width * (0.16 + Math.random() * 0.68);
        const y = height * (0.18 + Math.random() * 0.36);
        createFirework(x, y, colors[index % colors.length]);
        pulsePower = 0.72;
      }, index * 260);
    }
  }
  ...
  return { resize, animate, formText, pulse, spark, fireworks: launchFireworks };
})();
